...
const compareCoords = (c1: coordinate, c2: coordinate): boolean =>
  c1[0] === c2[0] && c1[1] === c2[1]
...
export const visitHomes = (inputs: string[]): number => {
  ...
  const currentHome: coordinate = [...initHome]
  inputs.forEach((char) => {
...
export const visitHomes2 = (inputs: string[]): number => {
  visitedHomes = [initHome]
  const currentHome1: coordinate = [...initHome],
    currentHome2: coordinate = [...initHome]
